...
from classifier_classes import LRClassifier
...
#---------------------------------
# 1.1 Save layer-wise accuracy
#---------------------------------

def save_layer_accuracy_csv(layer_results, model_name, classifier_type):
    """Save layer-wise accuracy results to CSV."""
    # Create DataFrame from layer results
    df = pd.DataFrame(layer_results, columns=['layer', f'acc_{classifier_type}'])
    df['model'] = model_name
    
    # Save to datasets directory
    csv_path = os.path.join(config.datasets_dir, f'training_accuracy_layers_{classifier_type}.csv')
    df.to_csv(csv_path, index=False)
    print(f"Saved layer accuracy results to: {csv_path}")

#---------------------------------
# 1.2 Add predictions to model knowledge files
#---------------------------------

def add_lr_predictions_to_model_knowledge_files(classifier, layer_idx, model_name, train_idx, test_idx):
    """Add LR classifier predictions and train/test split info to model knowledge files."""
    ...
    split_indicator = ['unknown'] * len(prob_positive)
    for i in train_idx:
        split_indicator[i] = 'train'
    for i in test_idx:
        split_indicator[i] = 'test'
    ...
    for dataset_name, version, start_idx, end_idx in dataset_info:
        ...
        dataset_split_info = split_indicator[start_idx:end_idx]
        
        # Add new columns
        mk_df['prob_lr'] = dataset_predictions
        mk_df['split'] = dataset_split_info
        ...
        print(f"  Updated: {dataset_name}_{version:03d}.csv with LR predictions and split info")
...
def train_lr_classifier():
    ...
    for layer_idx in available_layers:
        ...
        try:
            ...
            classifier = LRClassifier(config.normalize_activations)
            ...
            classifier.train(train_acts, torch.tensor(train_labels, dtype=torch.long), config.ridge_penalty)
    ...
    # Save layer accuracy results to CSV
    save_layer_accuracy_csv(layer_results, model_name, 'lr')
    
    # Save best classifier and add predictions to model knowledge files
    if best_classifier:
        config.save_classifier(best_classifier, f"lr_best", model_name)                
        print(f"Best LR classifier: Layer {best_layer}, Accuracy: {best_accuracy:.4f}")
        ...
        print(f"Adding LR predictions to model knowledge files using layer {best_layer}")
        add_lr_predictions_to_model_knowledge_files(best_classifier, best_layer, model_name, train_idx, test_idx)
