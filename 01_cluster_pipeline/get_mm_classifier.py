##########################################
# Train Mass Mean Difference Classifiers
##########################################
...
from classifier_classes import MMClassifier
...
def add_mm_predictions_to_model_knowledge_files(classifier, layer_idx, model_name, train_idx, test_idx):
    """Add MM classifier predictions to model knowledge files."""
    ...
    for dataset_name, version, start_idx, end_idx in dataset_info:
        ...
        # Add new column
        mk_df['prob_mm'] = dataset_predictions
        ...
        print(f"  Updated: {dataset_name}_{version:03d}.csv with MM predictions")

#---------------------------------
# 1.3 Train classifier
#---------------------------------

def train_mm_classifier():
    """Train mass mean difference classifiers."""
    ...
    # Convert labels to -1/1 format for MM classifier
    ...
    for layer_idx in available_layers:
        ...
        try:
            ...
            classifier = MMClassifier(config.normalize_activations)
            ...
            classifier.train(train_acts, torch.tensor(train_labels, dtype=torch.long))
    ...
    save_layer_accuracy_csv(layer_results, model_name, 'mm')
    
    # Save best classifier and add predictions to model knowledge files
    if best_classifier:
        config.save_classifier(best_classifier, f"mm_best", model_name)
        print(f"Best MM classifier: Layer {best_layer}, Accuracy: {best_accuracy:.4f}")
        ...
        print(f"Adding MM predictions to model knowledge files using layer {best_layer}")
        add_mm_predictions_to_model_knowledge_files(best_classifier, best_layer, model_name, train_idx, test_idx)
