...
#---------------------------------
# 1.1 Activation extraction for a batch
#---------------------------------
...
def process_dataset(dataset_name, dataset_version, model_name, model, tokenizer, device, padding_length, use_model_knowledge=False):
    ...
    all_activations = {layer: [] for layer in config.layers_to_extract}
    sentence_mapping = []
    ...
    for batch_num in range(n_batches):
        ...
        activations = extract_activations_for_batch(
            model, tokenizer, statements, device, config.layers_to_extract, padding_length
        )
