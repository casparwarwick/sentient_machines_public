...
#---------------------------------
# 1.2 Filter a dataset by confidence
#---------------------------------
...
def filter_dataset_by_confidence(dataset_name, model, tokenizer, device, true_token_id, false_token_id):
    ...
    for version in [1, 2, 3]:
        ...
        # Save filtered dataset into the model_knowledge dir.
        output_path = config.get_model_knowledge_dataset_path(dataset_name, version)
        df_filtered.to_csv(output_path, index=False)
        if version == 1:
            print(f"  Saved: {dataset_name}_{version:03d}.csv ({len(df_filtered)} rows, includes prob_true/prob_false)")
        else:
            print(f"  Saved: {dataset_name}_{version:03d}.csv ({len(df_filtered)} rows)")
...
def main():
    ...
    for dataset_name in config.base_training_datasets:
        if "deception_" in dataset_name:
            print(f"Skipping {dataset_name} (handled by filter_deception_hallucinations)")
            continue
