...
# SENTIENCE PROJECT
##########################################
# Combine per-model training prep_ files into combined_results_training.csv
##########################################
#
...
output_csv <- "combined_csvs/combined_results_training.csv"
...
for (r in seq_len(nrow(runs))) {
  ...
  for (i in 1:3) {
    ...
    file_path <- file.path(input_dir, run$dir,
                           paste0("new_training_17_", prompt_suffix, ".csv"))
